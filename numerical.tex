...
As expected, the rates of convergence shown in Figure \ref{error} are of order (in $h$) of 1/2. Indeed, since the notion of trace is not defined for the spaces $H^s_0(-L,L)$ with $s\leq 1/2$ (see \rouge{poner cita}), a behavior as in Fig. \ref{s01a} is somehow natural, so we cannot expect a point-wise convergence. 
%
\begin{figure}
 \centering
